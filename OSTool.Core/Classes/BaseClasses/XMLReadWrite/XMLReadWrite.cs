...
namespace OSTool.Core
{
    ...
    {
        public static string root = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData).ToString(), "OSTool.Core");
        public static string path = Path.Combine(root, "OSTool.Core.xml");
        ...
        public static void xmlCreate()
        {
            try
            {
                ...
                {
                    List<XElement> xmllist = XDocument.Load(path).Descendants("Data").DescendantNodes().OfType<XElement>().ToList();
                    foreach (var xml in xmllist)
                    {
                        if (xml.Parent.Name.ToString() == "Data")
                        {
                            if (xml.HasElements == false)
                            {
                                xmlRemove(xml.Parent.Name.ToString(), xml.Name.ToString());
                            }
                        }
                    }
                    if (xmlExist("ProgramOptions", "", 2) == false)
                    {
                        xmlWrite("ProgramOptions", "", "", 2);
                    }
                    if (xmlExist("ProgramOptions", "FirstTime", 0) == false)
                    {
                        xmlWrite("FirstTime", "True", "ProgramOptions", 1);
                    }
                    if (xmlExist("ProgramOptions", "DisclaimerAccepted", 0) == false)
                    {
                        xmlWrite("DisclaimerAccepted", "False", "ProgramOptions", 1);
                    }
                    if (xmlExist("ProgramOptions", "DisplayWarning", 0) == false)
                    {
                        xmlWrite("DisplayWarning", "True", "ProgramOptions", 1);
                    }
                }
            }
            ...
            {
                Log(error);
                throw new ApplicationException("XML creation failed. For more info, read the log(s).");
            }
        }
        ...
        public static void xmlRemove(string getDescendant, string childElement)
        {
            ...
            {
                Log(error);
                throw new ApplicationException("XML removal failed, closing now. For more info, read the log(s).");
            }
        }
