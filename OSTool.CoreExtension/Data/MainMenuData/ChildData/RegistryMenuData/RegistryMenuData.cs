...
namespace OSTool.CoreExtension
{
    public class RegistryMenuData
    {
        ...
        {
            xmlWrite("", "", "Registry", XmlWriteEnums.List);
        ...
        {
            xmlWrite(nameof(EventLog), type == 0 ? "True" : "False", "Registry", XmlWriteEnums.Element);
        ...
        {
            xmlWrite(nameof(ExecuteOptions), type == 0 ? "True" : "False", "Registry", XmlWriteEnums.Element);
            ProcessStart.Process(@"C:\Windows\SysWOW64\WindowsPowerShell\v1.0\powershell.exe", "powershell", type == 0 ? new string[] { "Set-ProcessMitigation -System -Disable DEP, SEHOP, AuditSEHOP, SEHOPTelemetry, CFG" } : new string[] { "Set-ProcessMitigation -System -Enable DEP, SEHOP, AuditSEHOP, SEHOPTelemetry, CFG" }, false); 
        }

        public static void Mitigations(int type)
        {
            xmlWrite(nameof(Mitigations), type == 0 ? "True" : "False", "Registry", XmlWriteEnums.Element);
        ...
        {
            xmlWrite(nameof(PwrThrottling), type == 0 ? "True" : "False", "Registry", XmlWriteEnums.Element);
        ...
        {
            xmlWrite(nameof(StartOptions), type == 0 ? "True" : "False", "Registry", XmlWriteEnums.Element);
            ProcessStart.Process(@"C:\Windows\System32\cmd.exe", "bcdedit", new string[] { type == 0 ? "/set useplatformtick yes" : "/deletevalue useplatformtick", type == 0 ? "/set disabledynamictick yes" : "/deletevalue disabledynamictick" }, false);
        }

        public static void SvcHostSplit(int type)
        {
            xmlWrite(nameof(SvcHostSplit), type == 0 ? "True" : "False", "Registry", XmlWriteEnums.Element);
            RegistryAdd(@"SYSTEM\CurrentControlSet\Control", "SvcHostSplitThresholdInKB", type == 0 ? Convert.ToInt32(ManagementObject.Query("SELECT * FROM Win32_OperatingSystem", "TotalVisibleMemorySize")) : 0x380000, CustomRegistryValueKind.DWord);
        }

        public static void SystemCache(int type)
        {
            xmlWrite(nameof(SystemCache), type == 0 ? "True" : "False", "Registry", XmlWriteEnums.Element);
        ...
        {
            xmlWrite(nameof(Win32Prio), type == 0 ? "True" : "False", "Registry", XmlWriteEnums.Element);
